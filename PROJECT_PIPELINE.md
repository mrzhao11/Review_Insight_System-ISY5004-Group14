...
| Zero-shot Flan-T5 title baseline | ROUGE-L F1 0.146 | 在同一个 61-row pseudo-title test set 上评估。 |
| Pseudo-label T5 student | ROUGE-L F1 0.334 | 用 Ark pseudo titles 微调后明显优于 zero-shot。 |
...
data/processed/
  ...
  pseudo_summary_manifest.json
    Ark 生成的 complaint-title pseudo labels，已经重新切分为 214/30/61。
...
models/
  ...
  t5_pseudo_summary_samples.json
    Ark pseudo-title 微调后的 T5 student。模型目录较大，被 .gitignore 忽略。
...
- 清洗 HTML、URL、空白字符。
- 过滤太短评论。
- 每个类目采样 800 条，共 4,000 条。
- 每个类目尽量保留 120 条 high-value 正样本。
- 加入商品标题 metadata。
...
negative: rating <= 2 and lex_score < 0.05
discard: otherwise
```
...
这个脚本不会调用 Ark，不会烧 API。它把已经生成好的 305 条 pseudo-title rows 合并、去重，然后重新切分，目的是扩大 test set，让 T5 评估更稳定。
...
这里虽然脚本名叫 `train_t5.py`，但它对 zero-shot baseline 不训练，只加载 `google/flan-t5-small` 直接生成，并在同一个 pseudo-title split 上评估。
...
| ROUGE-2 F1 | 0.165 |
| ROUGE-L F1 | 0.334 |
| Avg generated words | 4.84 |
| Avg reference words | 4.82 |

对比 zero-shot：

| Model | Test Rows | ROUGE-1 F1 | ROUGE-2 F1 | ROUGE-L F1 |
| --- | ---: | ---: | ---: | ---: |
| Zero-shot Flan-T5 | 61 | 0.156 | 0.060 | 0.146 |
| Pseudo-label T5 student | 61 | 0.353 | 0.165 | 0.334 |

推荐报告写法：

```text
After expanding the pseudo-title test set from 25 to 61 examples, the pseudo-label fine-tuned T5 student achieves ROUGE-L F1 of 0.334, compared with 0.146 for the zero-shot Flan-T5 baseline on the same test split.
```
...
对短标题任务，报告中建议主看 `ROUGE-L F1`，辅看 `ROUGE-1/2 F1`。
...
# 7. Fine-tune pseudo-label T5 student
.venv/bin/python -m src.summarization.fine_tune_t5_pseudo

# 8. Evaluate zero-shot T5 baseline on the same pseudo-title split
.venv/bin/python -m src.summarization.train_t5 \
  --train-file data/processed/pseudo_summary_train.csv \
  --validation-file data/processed/pseudo_summary_validation.csv \
  --test-file data/processed/pseudo_summary_test.csv \
