...
Dashboard 默认展示采样后的全量数据（train + validation + test），常量是：
...
1. `Business Overview`
   ...
   - 展示当前筛选 scope（默认全量样本）的 review 数、negative 数、high-value negative 数。
   - 展示类目分布、投诉关键词和生成标题样例。
...
7. 前端展示口径与模型评估口径不同：Dashboard 默认看 sampled full dataset（train+validation+test），而实验指标始终看 hold-out test。报告里请把这两件事明确分开写。
...
for path in [
    ...
    'models/t5_pseudo_summary_small_metrics.json',
    'models/t5_pseudo_summary_base_metrics.json',
]:
